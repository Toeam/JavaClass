...
public class NaverSQL {
	...
	// 1. DB접속 메소드
	public void connect() {
		con = DBC.DBConnect(); // DBC에있는 con의 정보들을 담으면서 DB에 접속이 된다.
	}
	
	// 2. DB접속해제 메소드
	public void conClose() {
		try {
			con.close(); // 처음부터 2번을 실행시키면 null값이기 때문에 오류가 생김
	...
	public void select() {
		...
		try {
			...
			rs = stmt.executeQuery(sql); // oracle에서의 sql문의 결과를 rs에 넣겠다.
