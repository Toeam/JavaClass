...
public class dealMain {
	...
	public static void main(String[] args) {
		...
		do {
			...
			System.out.println("1.홈         2.로그인/회원가입         3.내정보            4.로그아웃         5.입금");
			...
			case 7: // 글 목록
				...
				System.out.println("메인메뉴로 돌아가고싶으시면 0번을 입력해주세요\n");
				...
				if (dsql.showList(cateInput) != null) {
					System.out.printf("\n번호 %-6s%3s\n", "카테고리", "제목");
					...
					System.out.println("글을 보시고 싶으시면 번호를 선택해 주세요");
					System.out.println("메인메뉴로 돌아가고싶으시면 0번을 입력해주세요\n");
					System.out.print("번호 입력 : ");
					...
					if (pay == 1) {
						...
						if (dsql.payment(dc, Integer.parseInt(postList3[0]), payPW) == 1) {
							dsql.payComplete(postList2[1], postList2[2], postList3[1]);
							if (dsql.payComplete(postList2[1], postList2[2], postList3[1]) == 1) {
								System.out.println("\n\n정상적으로 결제가 완료되었습니다");
								dsql.payCal(postList2[2], postList3[0]);
							} else {
								System.out.println("\n\n결제에 실패하였습니다\n\n");
							}
			...
			case 8: // 글 수정
				if (id == null || pw == null) {
					...
					if (dsql.cntPost(id) >= 1) {
						dsql.Modify(id);
						System.out.println("\n수정하고싶은 글의 번호를 선택해 주세요");
						System.out.println("메인메뉴로 돌아가고싶으시면 0번을 입력해주세요\n");
						System.out.print("번호 입력 : ");
						int selectModifyList = sc.nextInt();

						if (selectModifyList == 0) {
							menu = 0;
							System.out.println("\n\n메인메뉴로 돌아가겠습니다\n\n");
							break;
						} else {
			...
			case 9: // 글 삭제
				if (id == null || pw == null) {
					...
					if (dsql.cntPost(id) >= 1) {
						dsql.Modify(id);
						System.out.println("\n삭제하고싶은 글의 번호를 선택해 주세요");
						System.out.println("메인메뉴로 돌아가고싶으시면 0번을 입력해주세요\n");
						System.out.print("번호 입력 : ");
						int selectDeleteList = sc.nextInt();

						if (selectDeleteList == 0) {
							menu = 0;
							System.out.println("\n\n메인메뉴로 돌아가겠습니다\n\n");
							break;
						} else {
							dsql.deletePost(id, selectDeleteList);
						}
